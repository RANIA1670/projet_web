...
if ((int) ($_SESSION['user_id'] ?? 0) <= 0) {
    header('Location: ' . cityzen_login_url(forum_list_url('page=create')), true, 302);
    exit;
}
...
cityzen_forum_public_shell_start('Nouvelle discussion — Forum');
?>
<style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .forum-create-wrap { max-width: 760px; margin: 0 auto; padding: 32px 20px 48px; }
...
document.getElementById('postForm').addEventListener('submit', function(e) {
    ...
    .then(data => {
        if (data.success) {
            window.location.href = <?= json_encode(forum_list_url('page=home'), JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT) ?>;
        } else {
