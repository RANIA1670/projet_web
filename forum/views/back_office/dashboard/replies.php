            ...
            if ($allReplies) {
                foreach ($allReplies as $item) {
                    ...
                    <tr id="reply-row-<?= (int)$reply->getId() ?>">
                        ...
                        <td>
                            <a href="<?= htmlspecialchars(forum_admin_nav_base(), ENT_QUOTES, 'UTF-8') ?>?page=edit_reply&id=<?= (int)$reply->getId() ?>&post_id=<?= (int)$item['post_id'] ?>" class="action-btn" style="display:inline-block;text-decoration:none;text-align:center;">✏️ Éditer</a>
