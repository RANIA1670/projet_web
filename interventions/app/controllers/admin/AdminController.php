...
class AdminController extends Controller
{
    ...
    {
        if (function_exists('cityzen_is_agent') && cityzen_is_agent()) {
            return;
        }
        $this->requireLogin();
    ...
    public function dashboard(array $params = []): void
    {
        ...
        $dbCit = Database::getInstance();
        $citoyens = (int) $dbCit->query("SELECT COUNT(*) FROM users WHERE role = 'user'")->fetchColumn();
        $techniciens = (int) $dbCit->query('SELECT COUNT(*) FROM interv_techniciens')->fetchColumn();
    ...
    public function showIntervention(array $params = []): void
    {
        ...
        if (!$intervention) {
            ...
            $this->redirect('/backoffice/interventions');
        }
    ...
    public function storeIntervention(array $params = []): void
    {
        ...
        if ($data['signalement_id'] <= 0) {
            ...
            $this->redirect('/backoffice/intervention/creer');
        }
    ...
    public function storeTechnicien(array $params = []): void
    {
        ...
        if ($this->userService->insertTechnicien($data) > 0) {
            $this->setFlash('success', 'Technicien ajouté.');
            $this->redirect('/backoffice/techniciens');
        } else {
            $this->setFlash('error', 'Erreur lors de l\'ajout.');
            $this->redirect('/backoffice/technicien/creer');
        }
    }
    ...
    {
        $id         = (int)($params['id'] ?? 0);
        $technicien = $this->userService->findTechnicienById($id);
        if (!$technicien) {
            $this->redirect('/backoffice/techniciens');
        }
    ...
    public function updateTechnicien(array $params = []): void
    {
        ...
        if ($this->userService->updateTechnicienRow($id, $data)) {
    ...
    public function deleteTechnicien(array $params = []): void
    {
        $id = (int)($params['id'] ?? 0);
        if ($id > 0) {
            $this->userService->deleteTechnicienById($id);
    ...
    public function exportPDF(array $params = []): void
    {
        ...
        $cit = (int) $dbx->query("SELECT COUNT(*) FROM users WHERE role = 'user'")->fetchColumn();
        $tech = (int) $dbx->query('SELECT COUNT(*) FROM interv_techniciens')->fetchColumn();
