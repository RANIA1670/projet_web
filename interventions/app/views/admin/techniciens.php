<div class="admin-container container py-5">
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        ...
        <a href="<?= APP_URL ?>/backoffice/technicien/creer" class="btn btn-primary btn-sm"><i class="fas fa-plus"></i> Ajouter un technicien</a>
    </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if (!empty($tech['is_busy']) && !empty($tech['current_intervention'])): ?>
                                            <a href="<?= APP_URL ?>/backoffice/intervention/<?= htmlspecialchars($tech['current_intervention']['id']) ?>" class="btn btn-sm btn-outline-secondary">#<?= htmlspecialchars($tech['current_intervention']['id']) ?></a>
                                        <?php else: ?>
                                            <span class="text-muted">Aucune</span>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <a href="<?= APP_URL ?>/backoffice/technicien/<?= htmlspecialchars($tech['id'] ?? '') ?>/edit" class="btn btn-sm btn-outline-secondary"><i class="fas fa-pencil-alt"></i></a>
                                        <form action="<?= APP_URL ?>/backoffice/technicien/<?= htmlspecialchars($tech['id'] ?? '') ?>/supprimer" method="POST" class="d-inline" onsubmit="return confirm('Supprimer ce technicien ?');">
