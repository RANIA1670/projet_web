    ...
    public function backEnvoyerRappels(): void
    {
        ...
        foreach ($events as $event) {
            ...
            foreach ($participants as $participant) {
                ...
                $headers = "From: CityZen <no-reply@cityzen.local>\r\n" .
                           "Reply-To: no-reply@cityzen.local\r\n" .
                           "Content-Type: text/plain; charset=UTF-8\r\n";

                $smtpHost = trim((string) getenv('CITYZEN_SMTP_HOST'));
                $smtpUser = trim((string) getenv('CITYZEN_SMTP_USER'));
                $smtpPass = (string) getenv('CITYZEN_SMTP_PASS');
                $smtpPort = max(1, (int) (getenv('CITYZEN_SMTP_PORT') ?: 587));
                $fromAddr = trim((string) (getenv('CITYZEN_SMTP_FROM') ?: $smtpUser));

                // PHPMailer uniquement si identifiants fournis dans l'environnement (storage/local.env)
                if (
                    $smtpHost !== ''
                    && $smtpUser !== ''
                    && $smtpPass !== ''
                    && $fromAddr !== ''
                    && is_file(__DIR__ . '/../phpmailer/PHPMailer-master/src/PHPMailer.php')
                ) {
                    ...
                    try {
                        ...
                        $failed++;
                        $errors[] = 'SMTP ' . $to . ' : ' . $e->getMessage();
                        error_log('CityZen événements — SMTP ' . $to . ' : ' . $e->getMessage());
                    }
                } elseif (@mail($to, $subject, $message, $headers)) {
                    $sent++;
                } else {
                    $failed++;
                    $errors[] = "Courrier PHP mail() impossible pour {$to}";
                    error_log('CityZen événements — mail() échec pour ' . $to);
                }
            }
        }
