...
class EventModel extends Model
{
    ...
    public function findAll(array $filters = []): array
    {
        ...
        if (!empty($filters['keyword'])) {
            ...
            $sql .= ' AND (LOWER(e.titre) LIKE :keyword OR LOWER(e.description) LIKE :keyword OR LOWER(e.lieu) LIKE :keyword OR LOWER(s.nom) LIKE :keyword)';
    ...
    public function findEventsInDays(int $days): array
    {
        ...
        $sql = 'SELECT e.*, s.nom AS nom_sponsor
                FROM event e
                JOIN sponsor s ON e.id_sponsor = s.id_sponsor
                WHERE e.date_event = DATE_ADD(CURRENT_DATE(), INTERVAL ' . $days . ' DAY)
