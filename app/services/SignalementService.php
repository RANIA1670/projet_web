<?php
/**
 * CityZen - SignalementService
 * Logique métier extraite de SignalementModel
 */
...
class SignalementService
{
    ...
    /** Accès direct au modèle (pour les opérations CRUD de base héritées) */
    ...
    /**
     * Récupère tous les signalements avec jointures catégorie + auteur,
     * filtrés et paginés.
     */
    ...
    /** Compte les signalements correspondant aux filtres */
    public function countFiltered(array $filters = []): int
    {
        ...
        $whereStr = implode(' AND ', $where);
        $stmt = $db->prepare("SELECT COUNT(*) FROM signalements s LEFT JOIN categories c ON s.categorie_id = c.id WHERE $whereStr");
        $stmt->execute($params);
        return (int)$stmt->fetchColumn();
    }

    /** Récupère un signalement avec tous ses détails (jointures) */
    ...
    {
        $db   = $this->model->getDb();
        $stmt = $db->prepare("SELECT statut, COUNT(*) as total FROM signalements GROUP BY statut");
        $stmt->execute();
        return $stmt->fetchAll();
    }

    /** Statistiques par catégorie (top 6) */
    ...
    /** Tendance des signalements des derniers jours */
    public function getTrendByDay(int $days = 7): array
    {
        $days     = max(1, $days);
        $interval = $days - 1;
        $db       = $this->model->getDb();
        $stmt     = $db->prepare("SELECT DATE(created_at) AS day, COUNT(*) AS total FROM signalements WHERE created_at >= DATE_SUB(CURDATE(), INTERVAL $interval DAY) GROUP BY day ORDER BY day ASC");
        $stmt->execute();
        $rows = $stmt->fetchAll();
    ...
    /** Récupère les N signalements les plus récents */
